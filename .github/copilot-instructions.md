...
- Use a group-level barrel: each component group (e.g., `presentational/index.ts`) is the canonical public surface and must re-export components directly from their .svelte files
- Re-export explicitly as named exports (e.g., `export { default as DatePickerDialog } from './DatePickerDialog.svelte';`)
- Export types alongside their components (e.g., `export { default as MenuButton, type MenuButtonItem } from './MenuButton.svelte';`)
- Only add a per-component `index.ts` when the folder needs to expose multiple symbols (component + helpers)
- **Avoid** barrel exports for:
  - Singleton components (to minimize circular reference risk)
  - Store files with parent/child relationships
  - Files that frequently cross-import each other
- When in doubt, run `pnpm check:circular` to detect circular dependencies
